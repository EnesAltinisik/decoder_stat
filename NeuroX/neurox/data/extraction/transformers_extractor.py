...
def aggregate_repr(state, start, end, aggregation):
    ...
    raise TypeError("Only first, last,average allowed")
...
def extract_sentence_representations(
    ...
    with torch.no_grad():
        ...
        if include_embeddings:
            all_hidden_states = [
                hidden_states[0].cpu().float().numpy() for hidden_states in all_hidden_states
            ]
        else:
            all_hidden_states = [
    ...
    # print("Detokenized (%03d) ind(%03d): %s" % (len(detokenized), 0, detokenized))
    
    #### llama-2 system prompt, we are removing the system prompt
    ind = [ind for ind,token in enumerate(detokenized) if '<0x0A><</SYS>><0x0A><0x0A>' in token][-1]+1
    if method is not None and method=='echo':
        ### close slotuion to find the second part of the prompt
        ind += (len(detokenized)-ind)//2+3
    # print("Counter: %d" % (counter))
    # print(detokenized[ind:])
    ...
    ### get the mean
    if last_mean=='mean':
        mean_emb = np.mean(final_hidden_states[:,ind:,:],axis=1)
        mean_emb = mean_emb.reshape(final_hidden_states.shape[0],1,final_hidden_states.shape[2]) ### for getting same shape
        return mean_emb, detokenized[-1:]
    
    ### get weighted embedding (tkn1*1+tkn2*2+...+tknN*N)/(1+2..+N)
    if last_mean=='weighted':
        embeddings = final_hidden_states[:,ind:,:]
        weights = np.arange(start=1, stop=embeddings.shape[1]+1)
        weights = np.array([weights]*embeddings.shape[0])
        sum_embeddings = np.sum(embeddings * np.expand_dims(weights, axis=-1),axis=1)
        sum_weights = np.sum(weights,axis=1)
        mean_emb = (sum_embeddings/np.expand_dims(sum_weights, axis=-1))
        mean_emb = mean_emb.reshape(final_hidden_states.shape[0],1,final_hidden_states.shape[2]) ### for getting same shape
        return mean_emb, detokenized[-1:]
    
    ### last word embedding
    else:
        return final_hidden_states[:,-1:,:], detokenized[-1:]
...
def extract_representations(
    ...
    for sentence_idx, sentence in tqdm(enumerate(corpus_generator(prompts))):
